...
export default async function merchantMediaRoutes(app) {
  ...
  // "purpose" field (cover|gallery|look|service|logo, defaults to gallery —
  // see media_uploads.purpose). Used by onboarding step 3's photo grid and
  // the Merchant Profile tab's "Add Studio Photo"/business logo flows,
  // either from the device's photo library or straight from the camera —
  // expo-image-picker hands both back as the same kind of file on the
  // client, so one endpoint covers both.
