...
export type ContactChannelKind =
  "whatsapp" | "call" | "website" | "instagram" | "tiktok" | "email";
...
export function buildContactChannels(
  ...
  const whatsappNumber = normalizeKenyanPhone(
    contacts.whatsapp || contacts.phone || provider.phone,
  );
  if (whatsappNumber) {
    channels.push({
      kind: "whatsapp",
      label: "WhatsApp",
      url: `whatsapp://send?phone=${whatsappNumber.replace("+", "")}`,
    });
  }
  ...
  if (isHttpUrl(contacts.website))
    channels.push({
      kind: "website",
      label: "Website",
      url: contacts.website.trim(),
    });
  if (isHttpUrl(contacts.instagram))
    channels.push({
      kind: "instagram",
      label: "Instagram",
      url: contacts.instagram.trim(),
    });
  if (isHttpUrl(contacts.tiktok))
    channels.push({
      kind: "tiktok",
      label: "TikTok",
      url: contacts.tiktok.trim(),
    });
  if (contacts.email)
    channels.push({
      kind: "email",
      label: "Email",
      url: `mailto:${contacts.email.trim()}`,
    });
